<template>
  <a-modal v-model="show" title="修改志愿申请" @cancel="onClose" :width="800">
  ...
  methods: {
    ...
    setFormValues ({...apply}) {
      ...
      Object.keys(apply).forEach((key) => {
        if (key === 'images') {
          this.fileList = []
          this.imagesInit(apply['images'])
        }
        // if (key === 'birthday' && apply[key] != null) {
        //   apply[key] = moment(apply[key])
        // }
