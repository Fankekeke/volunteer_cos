<template>
  <a-modal v-model="show" title="修改志愿专业" @cancel="onClose" :width="800">
  ...
  methods: {
    ...
    setFormValues ({...wishDiscipline}) {
      ...
      Object.keys(wishDiscipline).forEach((key) => {
        ...
        // if (key === 'birthday' && wishDiscipline[key] != null) {
        //   wishDiscipline[key] = moment(wishDiscipline[key])
        // }
    ...
    handleSubmit () {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          this.loading = true
          this.$put('/cos/wishDiscipline-info', {
            ...values
          }).then((r) => {
            this.reset()
            this.$emit('success')
